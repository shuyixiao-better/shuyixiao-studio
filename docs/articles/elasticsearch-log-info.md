...
#### v2025.6.1 (2026-01-17)

- ✅ 优化插件许可证到期时间提示
- ✅ 添加中英文语言显示支持

#### v2025.5.1 (2025-12-08)

- ✅ 修改日志监听器，避免提前返回以便检查完整curl命令
- ✅ 新增对包含curl和HTTP响应头的完整TRACE日志识别
- ✅ 重构缓冲区内容检查逻辑，统一使用bufferContent变量
- ✅ 引入时间戳解析功能，用于计算请求执行时间
- ✅ 增强执行时间提取机制，优先通过时间戳差值计算
- ✅ 添加日志时间戳正则匹配及日期格式化器
- ✅ 改进JSON结束符检测逻辑，提升解析准确性
